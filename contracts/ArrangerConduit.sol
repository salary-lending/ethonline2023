...
contract ArrangerConduit is IArrangerConduit {
    ...
    mapping(address => mapping(bytes32 => uint256))
        public
        override requestedFunds;
    mapping(address => mapping(bytes32 => uint256))
        public
        override withdrawableFunds;
    ...
    function setBroker(address broker, address asset, bool valid) external {
    ...
    function deposit(address source, address asset, uint256 amount) external {
        console.log("starting deposit");
        // deposits[asset][ilk] += amount;
        totalDeposits[asset] += amount;
        console.log("ArrangerConduit/deposit: %s", amount);

        // address source = RegistryLike(registry).buffers(ilk);
        console.log("ArrangerConduit/deposit source: %s", source);
    ...
    function withdraw(
        ...
        //     "ArrangerConduit/no-buffer-registered"
        // );
    ...
    function availableFunds(
        address asset
    ) public view override returns (uint256 availableFunds_) {
        console.log("runing availale funds");
        availableFunds_ =
    ...
    function getFundRequest(
        uint256 fundRequestId
    ) external view override returns (FundRequest memory fundRequest) {
    ...
    function isCancelable(
        uint256 fundRequestId
    ) external view override returns (bool isCancelable_) {
        isCancelable_ =
